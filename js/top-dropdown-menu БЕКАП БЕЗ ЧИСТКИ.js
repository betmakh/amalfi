...
(function () {
    ...
    if (Object.hasOwnProperty.call(h, "ActiveXObject") && !h.ActiveXObject) {
        o.ie = true
    }
    o[o.name] = true;
...
var euginDropdown = new Class({
    ...
    initialize: function (a) {
        this.setOptions(a);
        this._init_document_event();
        this._init_toggler_event()
    },
    _init_document_event: function () {
        document.addEvent("click", function (a) {
            if (a.target.hasClass(this.options.closeClass)) {
                this._hide_panels();
                return
            }
            if (a.target.hasClass(this.options.panel)) {
                return
            }
            if (a.target.getParent(this.options.container)) {
                return
            }
            if (this.options.keepOpen && a.target.getParents(this.options.keepOpen).getLast()) {
                return
            }
            this._hide_panels()
        }.bind(this))
    },
    _init_toggler_event: function () {
        var a = "click:relay(" + this.options.toggler + ")";
        document.getElement("body").addEvent(a, function (e, d) {
            e.preventDefault();
            var f = d.getParent(this.options.container);
            var b = f.hasClass(this.options.activeClass);
            var c = f.getElements(this.options.panel);
            this._hide_panels();
            if (!b) {
                this._show_panel(f, c)
            }
        }.bind(this))
    },
    _hide_panels: function () {
        document.getElements(this.options.container).removeClass(this.options.activeClass);
        document.getElements(this.options.panel).setStyle("display", "none")
    },
    _show_panel: function (b, a) {
...
$('.sideSliderParent').hover(function(){
	$(this).children('.sideSlider').fadeIn({duration:100,queue:false});

},function(){
	$(this).children('.sideSlider').fadeOut({duration:100,queue:false});
});
})